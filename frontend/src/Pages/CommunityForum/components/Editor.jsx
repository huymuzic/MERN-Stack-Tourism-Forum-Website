...
const MyEditor = (props) => {
   ...
   return (
      <div className="modal fade" id={`${status}Modal`} tabIndex="-1" aria-labelledby={`${status}ModalLabel`} aria-hidden="true">
         <div className="modal-dialog modal-md">
            <div className="modal-content">
               <div className="modal-header">
                  <h5 className="modal-title" id={`${status}ModalLabel`}>{status === 'post' ? 'New Post' : (status === 'reply' ? 'Replying' : 'Editing post')}</h5>
                  <button
                     type="button"
                     className="btn-close"
               ...
               <form onSubmit={handleSubmit(onSubmit)}>
                  <div className="modal-body">
                     {(status === 'post' || (status === 'edit' && post && post.parentId === null)) && <>
                        <input
                           ...
                           >
                              {selectedImages.map((image, index) => (
                                 <SwiperSlide key={index}>
                                    <img src={URL.createObjectURL(image)} alt={`image-${index}`} className='object-fit-contain img-fluid' />
                                    <button onClick={() => handleDelete(image)} className='ctm-btn rounded-5 position-absolute top-0 end-0 btn-index'>
